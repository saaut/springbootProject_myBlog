package org.myBlog.springbootdeveloper.controller.config.jwt;
...
public class TokenProviderTest {
    ...
    void generateToken(){
        //given
        User testUser = userRepository.save(User.builder().email("test@email.com")
                .password("test").build());
        //when
    ...
    void validToken_invalidToken(){
        ...
        String token = JwtFactory.builder()
                .expiration(new Date(new Date().getTime()-Duration.ofDays(7).toMillis()))//이미 만료된 토큰
                .build()
    ...
    @DisplayName("validToken():유효한 토큰인 때엔 유효성 검증에 성공한다.")
    @Test
